...
// ─── Client Optimization ─────────────────────────────────────────────────
    ...
    override fun searchAnimeParse(response: Response): AnimesPage =
        ...
        for (a in elements) {
            ...
            if (titleText.isBlank()) {
                ...
                titleText = container?.selectFirst("h1, h2, h3, h4, h5, .title, .anime-title, .name")
                    ?.text()?.trim() ?: ""
            }
    ...
    override fun animeDetailsParse(response: Response): SAnime {
        ...
        return SAnime.create().apply {
            title = doc.selectFirst(".anime-hero h1, h1.anime-title, [class*='anime-hero'] h1, [class*='detail'] h1, main h1, h1")
                ?.text()?.trim() ?: ""
            ...
            description = doc.selectFirst(".anime-synopsis, .synopsis, .description, [class*='synopsis'], [class*='description'], [class*='overview'], .desc")
                ?.text()?.trim()
            genre = doc.select(".genre-tag, .tag, a[href*='/genre/'], a[href*='/tag/'], [class*='genre'] a, [class*='tag']:not(script):not(style)")
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // EPISODE LIST + VIDEO LIST + FILTERS + PREFERENCES
    ...
    private fun extractFilenames(html: String): List<String> {
        ...
        val addDecoded = { fn: String ->
            ...
            if (cleanFn.isNotBlank() && !cleanFn.contains("/")) filenames.add(cleanFn)
        }

        val doc = Jsoup.parse(html)
        doc.select("a[href*='/download/'], a[data-filename], [class*='episode'] a, [class*='download'] a, table a, li a[href*='.mkv'], li a[href*='.mp4']")
            .forEach { el ->
                val href = el.attr("href")
                val dataFilename = el.attr("data-filename")

                when {
                    dataFilename.isNotBlank() -> addDecoded(dataFilename)
                    href.contains("/download/") -> addDecoded(href.substringAfterLast("/").substringBefore("?"))
                    href.endsWith(".mkv", ignoreCase = true) || href.endsWith(".mp4", ignoreCase = true) ->
                        addDecoded(href.substringAfterLast("/").substringBefore("?"))
                }
            }
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        return try {
            ...
            val tokenValue: String? = client.newCall(GET("$baseUrl/get_token", tokenHeaders))
                .execute().use { tokenResp ->
                    if (!tokenResp.isSuccessful) return@use null
                    val body = tokenResp.body.string()
                    try { json.decodeFromString<TokenResponse>(body).token } catch (_: Exception) { null }
                }

            val ddlUrlBuilder = "$baseUrl/get_ddl/$encodedFilename".toHttpUrl().newBuilder()
            tokenValue?.takeIf { it.isNotBlank() }?.let { ddlUrlBuilder.addQueryParameter("token", it) }
            ...
            val ddlResponse = client.newCall(GET(ddlUrlBuilder.build().toString(), ddlHeaders)).execute()
            ...
            val audioLabel = ddl.audioDetails?.audio?.mapNotNull { it.language }?.distinct()?.joinToString("/")
                ?.let { " [$it]" } ?: ""
    ...
    private class SortFilter : AnimeFilter.Select<String>(
        "Sort By", arrayOf("Latest Added", "A–Z", "Z–A", "Episode Count")
    )

    private class TypeFilter : AnimeFilter.Select<String>(
        "Audio Type (overrides Sort)", arrayOf("All", "Sub only (Airing)", "Dual audio (Airing)")
    )
    ...
    @Serializable private data class SubtitleInfo(
        @SerialName("format") val format: String? = null,
        @SerialName("language") val language: String? = null,
    )

    @Serializable private data class AudioDetailsWrapper(
        @SerialName("audio") val audio: List<AudioTrackInfo>? = null,
    )

    @Serializable private data class AudioTrackInfo(
        @SerialName("language") val language: String? = null,
        @SerialName("format") val format: String? = null,
